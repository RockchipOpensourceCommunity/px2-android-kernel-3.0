...
static struct list_head hdmi_display_device_list;
static struct list_head vga_display_device_list;
...
void rk_display_device_enable(struct rk_display_device *ddev)
{
	...
	if(ddev->property == DISPLAY_MAIN)
		head = &hdmi_display_device_list;
	else
		head = &vga_display_device_list;
	list_for_each(pos, head) {
		...
		if (!ret) {
			...
			if (!IS_ERR(new_dev->dev)) {
				...
				// Add new device to display device list.
				{
					...
					if(new_dev->property == DISPLAY_HDMI)
						head = &hdmi_display_device_list;
					else
						head = &vga_display_device_list;
					list_for_each(pos, head) {
...
static int __init rk_display_class_init(void)
{
	...
	INIT_LIST_HEAD(&hdmi_display_device_list);
	INIT_LIST_HEAD(&vga_display_device_list);
	return 0;
}
