			...
			if((hdmi_get_hotplug() == HDMI_HPD_ACTIVED) && (hdmi_switch_complete))
			{
				if(inf->num_fb >= 2)
				{
					...
					//	var->xres*var->yres*var->bits_per_pixel>>3);
						...
						#ifdef CONFIG_ARCH_RK3026 || CONFIG_ARCH_RKPX2
...
#else
	dev_drv = inf->lcdc_dev_drv[lcdc_id];
/*
...
*/
...
#ifdef CONFIG_RK_VGA 
	if((lcdc_id==0)&&rk29_get_vga_connect_status()){
		xpos =0;//((dev_drv->cur_screen->x_res - dev_drv->cur_screen->x_res*dev_drv->x_scale/100)>>1);
		ypos =0;//((dev_drv->cur_screen->y_res - dev_drv->cur_screen->y_res*dev_drv->y_scale/100)>>1);
		xsize = dev_drv->cur_screen->x_res; //* dev_drv->x_scale/100;
		ysize = dev_drv->cur_screen->y_res; //* dev_drv->y_scale/100;
		printk("rk_fb_switch_screen_vga %d,%d,%d,%d\n",xpos,ypos,xsize,ysize); 
		var = &info->var;
		var->nonstd &= 0xff;
		var->nonstd |= (xpos << 8) + (ypos << 20);
		var->grayscale &= 0xff;
		var->grayscale |= (xsize << 8) + (ysize << 20);
		printk("layer_id:%d,state:%d\n",layer_id,dev_drv->layer_par[layer_id]->state);
	...
	fix = &info->fix;
		...
		if(likely(inf->num_lcdc == 2))
		{
			...
			set_xact_yact_for_hdmi(pmy_var,var);
			var->nonstd &= 0xffffff00;
			var->nonstd |= (pmy_var->nonstd & 0xff); //use the same format as primary screen
		}
		else
		{
	...
	#endif
	var->grayscale &= 0xff;
	var->grayscale |= (dev_drv->cur_screen->x_res<<8) + (dev_drv->cur_screen->y_res<<20);
