...
#define IPP_VERSION "rkpx2-ipp 1.003"
...
int ipp_blit(const struct rk29_ipp_req *req)
{
	...
	uint32_t pre_scale_output_w=0, pre_scale_output_h=0;
	uint32_t post_scale_input_w, post_scale_input_h;
	...
	if(pre_scale)
	{
		if(((IPP_ROT_90 == rotate) || (IPP_ROT_270 == rotate)))
		{
			...
			{
				pre_scale_w = (uint32_t)( req->src0.w/req->dst0.h);
			}
			else
			{
			   pre_scale_w = 1;
			...
			{
				pre_scale_h = (uint32_t)( req->src0.h/req->dst0.w);
			}
			else
			{
				pre_scale_h = 1;
		...
		else
		{
			...
			{
				pre_scale_w = (uint32_t)( req->src0.w/req->dst0.w);
			}
			else
			{
			   pre_scale_w = 1;
			...
			{
				pre_scale_h = (uint32_t)( req->src0.h/req->dst0.h);
			}
			else
			{
				pre_scale_h = 1;
		...
		ipp_write((ipp_read(IPP_CONFIG)&0xffffffef)|PRE_SCALE, IPP_CONFIG); 		
	...
	{
		ipp_write(ipp_read(IPP_CONFIG)&(~PRE_SCALE), IPP_CONFIG); 
	...
	if((IPP_ROT_90 == rotate) || (IPP_ROT_270 == rotate))
	{
		if (( (req->src0.h%req->dst0.w)!=0)||( (req->src0.w%req->dst0.h)!= 0)
			||((req->src0.h/req->dst0.w)>8)||((req->src0.h/req->dst0.w)>8)	 
	...
	{
		if (( (req->src0.w%req->dst0.w)!=0)||( (req->src0.h%req->dst0.h)!= 0)
			||((req->src0.w/req->dst0.w)>8)||((req->src0.h/req->dst0.h)>8)	 
	...
	if(post_scale)
	{
		...
		ipp_write((ipp_read(IPP_CONFIG)&0xfffffff7)|POST_SCALE, IPP_CONFIG); 
		ipp_write((post_scale_h<<16)|post_scale_w, IPP_POST_SCL_PARA);
	}
	else 
	{
		DBG("no post_scale !!!!!! \n");
		ipp_write(ipp_read(IPP_CONFIG)&(~POST_SCALE), IPP_CONFIG); 
		ipp_write((post_scale_h<<16)|post_scale_w, IPP_POST_SCL_PARA);
	}
	...
	{
		ipp_write(ipp_read(IPP_CONFIG)&(~DEINTERLACE_ENABLE), IPP_CONFIG); 
	}
...
static int __devinit ipp_drv_probe(struct platform_device *pdev)
{
	...
	data->ipp_base = (void*)ioremap_nocache( RK29_IPP_PHYS, SZ_16K);
	...
	ret = request_irq(data->irq0, rk29_ipp_irq, 0, "rk29-ipp", pdev);
	if (ret)
	{
