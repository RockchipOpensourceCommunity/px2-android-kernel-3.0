...
unsigned int remote_filedata[44];
...
static int remotectl_read_filedata(struct rkxx_remotectl_drvdata  *ddata)
{
	char value[400];
	char keyValue[10];
	char *errp=NULL;
	...
	int* p = (int *)value;	
	long fd, fdir,i,j,k=0;
	int count=0,start=-1;
	int num=0;
	memset(value,0,400);
	fd = sys_open("/data/remotectl.dat", O_RDONLY ,0 );	
	
	if(fd < 0){		
		return -1;	
	}	
	sys_read(fd,(char __user *)value,400);	
	...
	while(value[count]!=0){
		...
		if(value[count]==0x2c){
			if(pstart>=0){
				memset(keyValue,'\0',10);
				memcpy(keyValue,&value[start],count-start);
				remote_filedata[num++]=simple_strtol(keyValue,errp,16);
			}
		}
	...
	remotectl_button[0].usercode=remote_filedata[0];
	remotectl_button[0].nbuttons=remote_filedata[1];
	for(i=0;i<22;i++){
		remotectl_button[0].key_table[i].scanCode=remote_filedata[i+2];
		remotectl_button[0].key_table[i].keyCode=remote_filedata[i+2];
	}
...
static void remotectl_delay_work(struct work_struct *work)
{
	struct rkxx_remotectl_drvdata  *ddata = container_of((work), \		
		struct rkxx_remotectl_drvdata, delay_work);

	if( remotectl_read_filedata(ddata) >= 0 ){
			return;
	}
	queue_delayed_work(ddata ->wq, &ddata ->delay_work, msecs_to_jiffies(1000));
}
...
static int __devinit remotectl_probe(struct platform_device *pdev)
{
	...
	ddata->wq = create_singlethread_workqueue("remote");
	INIT_DELAYED_WORK(&ddata->delay_work, remotectl_delay_work);
	queue_delayed_work(ddata->wq, &ddata->delay_work, msecs_to_jiffies(1000));	
